...
export const Highlights = ({ list, editMode }) => {
  ...
  const deleteListItem = (item) => {
    dispatch(actions.deleteHighlightItem(item._id));
  ...
  const MyComp = ({ item, style }) => {
    ...
    } else if (item.social && item.social.phone) {
      return (
        <div style={style}>
          <div
            onClick={() => {
              setOpen({ open: true, item: item.social });
            }}
    ...
    } else if (item.social && item.social.socialid) {
      return (
        <div style={style}>
          <div
            onClick={() => {
      ...
      {highList && highList.length !== 0 ? (
        <>
          ...
          >
            ...
            <DialogContent>
              {open.item && open.item.phone && (
                <p>
                  {" "}
                  Number: <b> {open.item && open.item.phone}</b>
                </p>
              )}
              {open.item && open.item.socialid && (
                <p>
                  {" "}
                  Socialid: <b> {open.item && open.item.socialid}</b>
                </p>
              )}
            </DialogContent>
            <DialogActions>
        ...
        <div className="wrap-window-empty">
          <div className="fixed-top-window">
            <WhatshotIcon className="hot-icon" />
            <span>Highlights</span>
          </div>
          <div className="text-empty-hs">
            <p>No highlights added - Click edit profile button to add</p>
            <small>Highlights are not showing on profile page when empty</small>
          </div>
        </div>
      )}
    </>
